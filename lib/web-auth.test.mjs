...
// The ambient PI_WEB_PASSWORD (exported for the dev server) must not leak into
// the "unset" semantics these tests exercise.
const ambientPassword = process.env.PI_WEB_PASSWORD;
delete process.env.PI_WEB_PASSWORD;
after(() => {
  if (ambientPassword !== undefined) process.env.PI_WEB_PASSWORD = ambientPassword;
});
