...
<<<<<<< HEAD
test("allows the Google OAuth callback navigation, and nothing else cross-site", async () => {
  const { isApiRequestAllowed } = await loadSubject();
  // What Google's redirect actually looks like: a cross-site document
  // navigation that no same-origin check can ever pass.
  ...
  assert.equal(isApiRequestAllowed(new Request(
    "http://127.0.0.1:30141/api/robin/google/callback?code=abc&state=xyz",
    { headers: callbackHeaders },
  )), true);

  // The exemption is one path and one method; the `state` nonce is what
  // actually authenticates the callback.
  assert.equal(isApiRequestAllowed(new Request(
    "http://127.0.0.1:30141/api/robin/google",
    { headers: callbackHeaders },
  )), false);
  assert.equal(isApiRequestAllowed(new Request(
    "http://127.0.0.1:30141/api/robin/settings",
    { headers: callbackHeaders },
  )), false);
  assert.equal(isApiRequestAllowed(new Request(
    "http://127.0.0.1:30141/api/robin/google/callback",
    { method: "POST", headers: callbackHeaders },
  )), false);

  // A hostile host header must still lose, exemption or not.
  assert.equal(isApiRequestAllowed(new Request(
    "http://127.0.0.1:30141/api/robin/google/callback?code=abc&state=xyz",
    { headers: { ...callbackHeaders, host: "evil.example.com" } },
  )), false);
=======
