...
已有会话可以在其工作目录中打开交互终端标签。由于这会让浏览器获得与 Pi Web 进程相同的文件系统和命令权限，终端路由采用比应用其他部分更严格的策略：必须设置 `PI_WEB_PASSWORD`、只接受回环地址请求，并且不能通过局域网或反向代理启用。关闭终端标签会终止 Shell；仅隐藏右侧面板不会终止。

只有当前正在查看的终端会保持连接——浏览器对同一来源的并发流限制为六条，因此切到后台的终端会断开连接，回到该标签时再补齐这期间缓冲的输出。无人查看的 Shell 会在 15 分钟后回收；正在打开的终端则不会被回收，无论静置多久。

Shell 以刻意精简的环境启动（不加载 `~/.bashrc` 和 `~/.profile`），因此其中没有 `nvm`、`pyenv` 和 Shell 别名；`PATH`、`HOME` 与 `SSH_AUTH_SOCK` 会被继承，以保证 `git` 走 SSH 时可用。终端依赖可选的原生包 `node-pty`：如果所在平台既没有预编译包也没有编译器，Pi Web 的其余功能照常运行，终端按钮会说明不可用的原因。

```bash
PI_WEB_PASSWORD='足够长的随机密码' pi-web
```

Shell 启动时不会加载用户启动脚本，并且只继承精简后的环境变量，但它不是操作系统级沙箱。在其中输入的命令仍拥有运行 Pi Web 的用户权限。
