...
function DismissButton({ onClick, title }: { onClick: () => void; title: string }) {
  return (
    <button
      ...
      title={title}
      aria-label={title}
      style={{ width: 24, height: 24, padding: 0, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, border: "none", borderRadius: 0 }}
  ...
  return (
    <div style={{ minHeight: "100%" }}>
      ...
      {showUploadFeedback && (
        ...
        {pendingConflict && (
          <div role="alert" style={{ padding: 7, border: "1px solid color-mix(in srgb, var(--warning) 55%, var(--border))", borderRadius: 0, background: "color-mix(in srgb, var(--warning) 9%, var(--bg-panel))" }}>
            ...
            <div style={{ display: "flex", gap: 5, marginTop: 7 }}>
              <button type="button" onClick={() => void performUpload(pendingConflict.files, "overwrite")} style={{ height: 22, padding: "0 7px", border: "1px solid var(--danger)", borderRadius: 0, background: "transparent", color: "var(--danger)", cursor: "pointer", fontSize: 10 }}>
                {t("files.replace")}
              </button>
