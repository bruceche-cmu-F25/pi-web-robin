...
Every Chinese browser locale — bare `zh`, Simplified and Traditional regions
alike — resolves to `zh-CN`. There is no Traditional Chinese package; a stored
`zh-TW` preference from before it was removed is read as `zh-CN`.
