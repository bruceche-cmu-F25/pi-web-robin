...
- **编程工作台** 在 `/coding` —— 内嵌 NeetCode 路线图，旁边坐着一个只给提示、不给答案的
  教练，并带你自己的记录。
- **Full Stack Open** 在 `/learn/fso` —— 整门课做成路线图；每一节内嵌在中间，旁边是你的
  笔记和一个知道你打开了哪一节的导师。
...
| `fso_current` | "这一节要我做出什么？" |
| `fso_notes` | "按我 Part 3 的笔记考考我" |
...
- **相对日期按本地日期解析**：每轮消息都会注入当前日期、时间、时区、星期及本周末的
  具体日期；与待办、日历工具一样使用运行 Robin 的主机本地时区，请确保主机时区正确。
- **一段交流一个会话**：连续追问保留上下文，闲置满 30 分钟或跨本地日期后，下一条消息
  自动新开；正在运行的任务不会被切断。网页和 Telegram 共享这个规则，`/reset` 可提前重开。
  教练、导师及产品助手也使用相同的切换规则；日报、邮件检查和岗位评分每次单开。
- **每个自然月清理一次旧会话**：服务启动时补跑当月未执行的清理，之后在月初执行；
  开发热更新后也会在下一条 Robin 消息时初始化，无需专门重启。
  执行月份持久化，重启不会重复清理。只删除最后活动超过 30 天、已登记归 Robin 管理的
  会话文件；当前引用的会话及仍存活的运行实例始终保留。不动普通编程会话和业务数据。
  升级前无法确认归属的旧会话不会被猜测删除。服务关闭期间不执行，下次启动补跑。
...
`/learn` 是入口：今天的两条轨道（NeetCode 和 Full Stack Open）、其他入口，以及学习链接
书架。

**书架是一份阅读清单，不是课程。** `learning-shelf.ts` 保留这份清单最初被收集时的分组，那是
你**已经知道要找什么**时找回它的方式。每个链接都在新标签页打开——这些站点大多（包括
freeCodeCamp）拒绝被内嵌。仪表盘上保存的链接是另一回事，仍留在原处。

`/coding` 是刷题：路线图 + NeetCode 本体 + 教练。打开的是哪道题是服务端状态，因为教练要
读它。旧的 `/coding?track=curriculum` 链接会跳到 `/learn/fso`。
...
`/learn/fso` 打开是一张 roadmap.sh 式的路线图：各 Part 在一条主线上，每一节挂在左边，
这一节的练习挂在右边。点开一节，fullstackopen.com 内嵌在中间，旁边是笔记和导师，左侧
目录默认收起。Part 8–14 已迁到 courses.mooc.fi，对方拒绝被内嵌，在新标签页打开。

**勾选就是仪表盘上的课程进度。** `fso.ts` 用和 `fullstack-open-progress.json` 相同的生成
目录和 step id 组织 Part 和章节，所以这里勾一道练习，仪表盘上的「继续」也跟着走。

**笔记一节一份**，输入时自动保存，在笔记本里按课程顺序汇总。导师的回答可以一键存进笔记，
笔记也可以发给导师检查。

**导师负责讲清楚，教练负责不讲。** 两件相反的事，所以跑在各自的会话里、各有各的工具。
`fso_current` 告诉导师打开的是哪一节、你自己在练习上的勾选和这一节的笔记；`fso_notes`
跨章节读笔记本。两者都不能写。

**存储**只有一个文件 `~/.pi/robin/fso.json`：最近打开的章节（frame 是跨域的，点击是导师
知道"这一页"是什么的唯一途径），以及笔记。

### 分栏宽度

每条缝都能调：拖它、双击复位、或者聚焦后用方向键微调。宽度按浏览器保存，刷题和
Full Stack Open 共用——读书时拉宽的面板，回去刷题还是那么宽。**缝之间
