/**
 * Refresh the Learning Hub's link logos.
 *
 ...
 * requests announcing, every time the Learning Hub opens, exactly which
 * tutorials someone keeps on their shelf. Google's favicon endpoint is the
...
const hosts = [
  ...new Set(
    learningShelf().flatMap((group) => group.entries.map((entry) => new URL(entry.url).host)),
  ),
].sort();
